...
function writef32array(folder,filename) {
  ...
  var outfilename = folder+filename+".float64array";
  ...
  fs.readFile(infilename, 'utf8', function (err, readdata) {
    ...
    var data = new Float64Array(dataarray);
    //var buffer = new Buffer(data.length*8); //8 bytes per float aka 64bit per float
    var buffer = Buffer.alloc(data.length*8);
...
/*
for (var n=0; n<betanames.length; n++) {writef32array("beta1/",betanames[n])} //shakespeare model dont have Wp etc, only component stuff like Wm
...
for (var n=0; n<alfanames.length; n++) {writef32array("beta1/",alfanames[n])} //so use these instead
for (var n=0; n<alfanames.length; n++) {writef32array("beta2/",alfanames[n])}
for (var n=0; n<alfanames.length; n++) {writef32array("beta3/",alfanames[n])}